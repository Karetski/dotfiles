...
**Section layout** — `_log_group` renders a single-line `═ name ═══…` divider above each cluster of related roles (`preflight`, `shell`, `cli tools`, `dev tools`, `system`, `toolchains`, `editor`). `_log_section` prints the role title with an optional `[n/total]` counter, and `_log_summary` prints the final totals.
